...
public:
    ...
    QVariant data(const QModelIndex& index, int role = Qt::DisplayRole)const override {
        if (role == Qt::DisplayRole)
        {
            ...
            if (index.column() == 2)
                return QString::number(a.getProg().size());
            if (index.column() == 3)
                return QString::fromStdString(a.getStare());
        }
...
private:
    ...
    {   
        ...
        model1->setElems(s.filtrats("open"));
        model2->setElems(s.filtrats("inprogress"));
        model3->setElems(s.filtrats("closed"));
        auto btn_1_o = new QPushButton{"Open"};
        auto btn_1_c = new QPushButton{"Closed"};
        auto btn_1_i = new QPushButton{"Progress"};
        auto btn_2_o = new QPushButton{ "Open" };
        auto btn_2_c = new QPushButton{ "Closed" };
        auto btn_2_i = new QPushButton{ "Progress" };
        auto btn_3_o = new QPushButton{ "Open" };
        auto btn_3_c = new QPushButton{ "Closed" };
        auto btn_3_i = new QPushButton{ "Progress" };
    ...
    void filtrare()
    {
        ...
        if (cautare->text().toStdString() == "")
            v = s.getAll();
        else v = s.filtrat(cautare->text().toStdString());
        model->setElems(v);
    }
    void add()
    {
        ...
        try {prof4->text().toStdString();
            s.add(ids, descrieres, prof1s, prof2s, prof3s, prof4s, stares);
        }
        catch (exception& e)
        {
    ...
    void update() override {
        model1->setElems(s.filtrats("open"));
        model2->setElems(s.filtrats("inprogess"));
        model3->setElems(s.filtrats("closed"));
        filtrare();
    }
